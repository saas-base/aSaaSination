...
namespace Core\Generator\Factories;
...
class ModuleFactory
{
    ...
    private function createModule($moduleName): Module
    {
        ...
        $path       = Api::getModulesBasePath() . '/' . $moduleName;

        if (file_exists($path)) {
            throw new FileAlreadyExistException('Module exists already. Please remove the directory first');
        }

        return new Module($moduleName, Api::getModulesBasePath() . '/' . $moduleName);
    }
    ...
    public function build()
    {
        foreach ($this->pipeline as $command) {
            ...
            call_user_func_array([$manager, $method], $command['arguments']);
        }
    }
